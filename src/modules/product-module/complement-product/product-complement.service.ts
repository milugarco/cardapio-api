...
export class ProductComplementService {
  constructor(private model: PrismaService, private productService: ProductService, private complementService: ComplementService) {}

  async create (createProductComplementDto: CreateProductComplementDto): Promise<ProductComplementResponse> {
    try {
      ...
      const complementExists = await this.complementService.findOne(complementId)

      if (!complementExists) {
        throw new NotFoundException('Complement not found or not exist');
      }

      const productComplementAlreadyExists = await this.model.productComplements.findFirst({
        where: {
      ...
      const productComplementCreated = await this.model.productComplements.create({
        data: {
          productsId: productId,
          complementsId: complementId,
        },
      ...
      throw new ConflictException(`Error creating product complement: ${error}`);
    }
  }

  async createMany(createProductComplementsDto: CreateProductComplementsDto): Promise<ProductComplementResponse[]> {
  try {
    ...
    const complementsExistencePromises = complementsId.map(async (complementId) => {
      const existingProductComplement = await this.productComplementExist(productId, complementId);
      return existingProductComplement;
    });

    const complementsExistenceResults = await Promise.all(complementsExistencePromises);
    const filteredComplementsId = complementsId.filter((complementId, index) => !complementsExistenceResults[index]);

    if (filteredComplementsId.length === 0) {
      throw new ConflictException('All provided complements are already associated with the product');
    }

    await Promise.all(filteredComplementsId.map(async (complementId) => {
    ...
    const productComplementsCreated = await this.model.productComplements.findMany({
      where: {
        productsId: productId,
        complementsId: {
  ...
  async findOne(id: number): Promise<ProductComplementResponse> {
    try {
      ...
      throw new NotFoundException(`Error fetching product complement: ${error}`)
    }
  }

  async findAll(productId?: number, complementId?: number, page?: number, perPage?: number): Promise<ProductComplementsResponse> {
    try {
  ...
  async update(id: number, updateProductComplementDto: UpdateProductComplementDto): Promise<ProductComplementResponse> {
    try {
      ...
      const complementExist = await this.complementService.findOne(Number(complementId))

      if(!complementExist) {
        throw new NotFoundException('Complement not exist');
      }

      const productComplementAlreadyExist = await this.productComplementExist(productComplementExist.productsId, complementId)

      if (productComplementAlreadyExist) {
        throw new ConflictException('This complement already exist in the product')
      }

      const productComplementUpdated = await this.model.productComplements.update({
        where: {id: Number(id)},
        data: {
          complementsId: complementId,
        },
      ...
      throw new ConflictException(`Error updating product complement: ${error}`)
    }
  }

  async productComplementExist(productId: number, complementId: number): Promise<ProductComplementResponse> {
    try {
       const existingProductComplement = await this.model.productComplements.findFirst({
        where: {
          productsId: productId,
          complementsId: complementId,
        }
